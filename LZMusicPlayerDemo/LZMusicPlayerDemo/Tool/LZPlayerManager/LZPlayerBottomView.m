...
//    self.progressSlider.hidden = YES;
    ...
    //圆圈
    self.circleView = [[HWCircleView alloc] init];
...
-(void)timerAct{
    ...
    self.circleView.totalProgress = totalTime;
    self.circleView.progress = currentTime;//当前播放进度
    if (self.circleView.progress == totalTime) {//如果progressSlider的值=总时长 就直接下一首(或者单曲循环)
        [self autoNext];
    }
